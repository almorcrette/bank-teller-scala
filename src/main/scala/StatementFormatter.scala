...
object StatementFormatter {
  ...
  private def formatTransactions(transactions: Seq[Transaction]): String = {
    ...
    @tailrec def recursiveFormatter(statementBuilder: mutable.StringBuilder, startingBalance: Int, transactionsCountDown: Int): String = {
      if (transactionsCountDown == 0) statementBuilder.toString
      else {
        val currentTransaction = transactionsReversed(transactionsCountDown - 1)
        recursiveFormatter(
          statementBuilder.addString(new mutable.StringBuilder(formatLine(currentTransaction.amount, currentTransaction.date, startingBalance + currentTransaction.amount))),
